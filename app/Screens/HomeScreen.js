...
const Home = ({navigation}) => {
  useEffect(() => {
    db.transaction(function (txn) {
      txn.executeSql(
        "SELECT name FROM sqlite_master WHERE type='table' AND name='table_event_1'",
        [],
        function (tx, res) {
          ...
          if (res.rows.length == 0) {
            ...
            txn.executeSql(
              'CREATE TABLE IF NOT EXISTS table_event_1(event_id INTEGER PRIMARY KEY AUTOINCREMENT, event_name VARCHAR(20), event_date INT(10), event_time INT(10), event_address VARCHAR(255), event_description VARCHAR(255))',
              [],
            );
          }
        },
